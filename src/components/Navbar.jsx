  ...
  return (
    <motion.header
      ...
      className={`fixed inset-x-0 top-0 z-50 transition-[background-color,backdrop-filter,border-color,padding] duration-500 ease-premium ${
        scrolled
          ? 'bg-forest-dark/90 backdrop-blur-xl border-b border-white/10 py-3'
    ...
    >
      <nav className="mx-auto flex max-w-[1400px] items-center justify-between pl-4 pr-2 sm:px-8">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 pr-1 sm:pr-0">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full text-white ring-1 ring-inset ring-white/20 transition-colors duration-300 hover:bg-white/10 lg:hidden shrink-0"
          >
      ...
      >
        <ul className="flex flex-col gap-1 bg-forest-dark/95 px-6 py-5 backdrop-blur-xl border-t border-white/10">
          {NAV_LINKS.map((link) => (
            <li key={link.label}>
              <a
                href={link.href}
          ...
          <li className="pt-3">
            <a
              ...
              className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-gold-dark via-gold to-gold-light px-6 py-3 text-center font-sans text-xs font-extrabold uppercase tracking-[0.14em] text-forest-dark shadow-[0_4px_15px_rgba(212,175,55,0.35)] border border-gold-light/50"
            >
